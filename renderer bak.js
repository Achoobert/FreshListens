...
const client = remote.require("./appLogic.js");
// const Square = require('./square.js');
// const mySquare = new Square(2);
// console.log(`The area of mySquare is ${mySquare.area()}`);
const Client = remote.require("./appLogic.js");
const user = new Client(2);
console.log(`an app variable is ${user.history()}`);

console.log(client.hi());
console.log(client.testCaller("renderer here, "));
//console.log(client.echo("test text"));
client.echo("pyhyhasr5tg", (error, res) => {
  console.error(res);
  console.log(client.hi());
  console.log(client.testCaller("renderer here, "));
});
  ...
  console.log(
    client.invoke("echo", "echoing with invoke", (error, res) => {
      if (error) {
        ...
        // make the user table
        console.log("helphelphelphelphelphelphelphelp");
      }
    })
  );
});
...
function libraryDatabaseRecheck() {
  return new Promise((resolve) => {
    client.libraryDatabaseRecheck((error, res) => {
      if (error) {
        console.error(error);
        // retry
...
scanLibrary.addEventListener("click", () => {
  console.log("click");
  debugger;
  let result = client.libraryDatabaseRecheck((error, res) => {
    if (error) {
      console.error(error);
    } else {
      console.log(res);
